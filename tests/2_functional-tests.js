...
suite('Functional Tests', function () {
  ...
  suite('Integration tests with chai-http', function () {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function () {
      ...
      test('Test GET /hello with your name', function (done) { // Don't forget the callback...
        chai.request(server) // 'server' is the Express App
          .get('/hello?name=jeanine') /** <=== Put your name in the query **/
          .end(function (err, res) { // res is the response object
            ...
            assert.equal(res.status, 200);
            assert.equal(res.text, 'hello jeanine' /** <==  Put your name here **/ );
            done(); // Always call the 'done()' callback when finished.
          });
      });
      ...
      test('send {surname: "Colombo"}', function (done) {
        ...
        chai.request(server)
          ...
          .end(function (err, res) {
            ...
            assert.equal(res.body.name, 'Cristoforo', 'name should return Cristoforo'); 
            assert.equal(res.body.surname, 'Colombo', 'surname should return Colombo'); 
            assert.equal(res.body.dates, '1451 - 1506', 'dates should return 1451 - 1506'); 
            assert.typeOf(res.body.dates, 'string', 'dates should return a string'); 
  ...
  Browser.site = 'https://fcc-boilerplate-mochachai.herokuapp.com/';
  // Browser.localhost('https://fcc-boilerplate-mochachai.herokuapp.com/', (process.env.PORT || 3000));
